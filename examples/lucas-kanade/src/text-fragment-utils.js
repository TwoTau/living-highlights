 ...
 */
 ...
 export const markRange = (range, documentToProcess = document) => {
   ...
   const hasParent = (node) => {
    ...
    return hasParent(node.parentNode) && !(node.nodeName === 'CITE-REF' || node.nodeName === 'CROSS-REF' || (node.className && node.classList.contains('enclosing')))
    }
   const walker = documentToProcess.createTreeWalker(
       ...
       NodeFilter.SHOW_ELEMENT | NodeFilter.SHOW_TEXT,
       {
         acceptNode: function(node) {
           if (!range.intersectsNode(node) || !hasParent(node)) return NodeFilter.FILTER_REJECT;
   ...
   return [startMark, ...marks, endMark].filter(m => {
  ...
  */
 export const getAllTextNodes = (root, range) => {
   const blocks = [];
   let tmp = [];
   ...
   const nodes = Array.from(
       getElementsIn(
           root,
           (node) => {
             return node.className !== 'katex-mathml' && acceptNodeIfVisibleInRange(node, range);
           }),
   );
